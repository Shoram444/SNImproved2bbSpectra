...
function BBB(
    ...
    minEvents = round(get_min_value(matStatsMr), digits = 2)                         # find minimum value
...
function get_needed_statistics(
    h1::Hist1D,
    h2::Hist1D,
    xMin::Real,
    xMax::Real,
...
function get_r_max_map(
    ...
    for minROI = xMin:stepSize:xMax-stepSize        # iterating over the range xMin, xMax
        for maxROI = minROI+stepSize:stepSize:xMax
            rmax =
                get_r_max(h1, h2, minROI, maxROI, stepSize) <= 1.0 ?
...
function get_min_stats_map(
    ...
    for minROI = xMin:stepSize:xMax-stepSize        # iterating over the range xMin, xMax
        for maxROI = minROI+stepSize:stepSize:xMax
            ...
            matRatios[Int(maxROI / stepSize), Int(minROI / stepSize)+1] =
                rmax <= 1.0 ?
                get_needed_statistics(h1, h2, minROI, maxROI, stepSize, nSigma) : NaN
        end
    end
